...
4. Configura las variables de entorno del Backend copiando la plantilla base:
   ...
   - **En Windows (PowerShell):**
     ...
     ```
   - _Nota: Puedes configurar tus credenciales de API directamente en el archivo `.env` o dejarlas en blanco y cargarlas dinámicamente desde la interfaz web más adelante._
...
6. Inicializa el servidor de desarrollo con Uvicorn:
   ...
   ```
   _El servidor backend quedará disponible en `http://127.0.0.1:8020`. Puedes auditar la documentación Swagger de la API en `http://127.0.0.1:8020/docs`._
...
3. Configura el entorno local del frontend:
   ...
   - **En Windows (PowerShell):**
     ...
     ```
   - _Verifica que la variable `NEXT_PUBLIC_API_BASE_URL` apunte correctamente al puerto asignado al Backend (`http://127.0.0.1:8020`)._

4. Despliega la interfaz web en modo desarrollo:
   ```bash
   npm run dev
   ```
   _Abre tu navegador e ingresa a `http://localhost:3000` para interactuar con la aplicación._
